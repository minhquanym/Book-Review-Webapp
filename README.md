# Project 1: Book Review

This is a homework project of my web programming courses.

Use Flask and PostgresSQL (Heroku database) as backend server.

Books data from goodread.

# Deploy to heroku
<a href="project1-cs50-minhquan.herokuapp.com">Book Review</a>


You can register for a new account, login and leave comment about books
Here is some images about my webapp

<img src="images/preview1.png" width="70%">

<img src="images/preview2.png" width="70%">

# Install:
  pip install -r requirements.txt
  
  
Web Programming with Python and JavaScript
